...
import storage, { sanidateString, sanidateURL } from "src/utils/config-storage";

const EXPLORERS = {
  explorer: { uri: "https://solscan.io/tx", label: "Solscan" },
  solanafm: { uri: "https://solana.fm/tx", label: "SolanaFM" },
  xray: { uri: "https://xray.helius.xyz/tx", label: "Xray (Helius)" },
};

const FALLBACK_EXPLORER = EXPLORERS.explorer.uri;

const PERFORMANCE_FEES = [0, 5e-6, 5e-4];
const SLIPPAGES = [0, 0.1, 0.5, 1, 2];
const FALLBACK_SLIPPAGE = SLIPPAGES[2];
const FALLBACK_VERSIONED_API = 0;
...
export default function useTxRunner() {
